...
- The instance remained in the "running" state with passing status checks, but was effectively unresponsive to users.
...
*Volume average throughput — sustained at the 125 MiB/s gp3 ceiling from ~07:25 UTC onward.*

![VolumeThroughputExceededCheck transitioning from 0 to 1](CloudWatchGraphs/EBS/VolumeThroughputExceededCheck.png)
*`VolumeThroughputExceededCheck` — the single clearest signal of the incident, transitioning from 0 to 1 at 07:25 UTC and remaining asserted until reboot.*

![Read/write latency spike during throttling](CloudWatchGraphs/EBS/VolumeAvgRead+WriteLatency,.png)
*Volume read/write latency spiking as queue depth climbs under sustained throttling — the downstream effect that cascaded into DBus, Docker, and SSH failures.*
...
- Status checks passed throughout, masking severity
...
| 6   | Configure Docker daemon log rotation (`max-size 10m`, `max-file 3`) — preventive hygiene; addresses adjacent      | P2       | ✅     |
|     | disk-fill failure mode                                                                                            |          |        |
...
4. **Infrastructure "healthy" ≠ application healthy.**
   - AWS status checks passed the entire incident.
